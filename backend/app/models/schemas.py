...
class TripWorkflowResponse(BaseModel):
    """支持人工中断与恢复的旅行规划工作流响应"""
    success: bool = Field(..., description="当前处理是否成功")
    status: Literal[
        "awaiting_approval",
        "completed",
        "cancelled",
        "failed",
    ] = Field(..., description="工作流状态")
    thread_id: str = Field(..., description="用于恢复工作流的线程ID")
    message: str = Field(default="", description="消息")
...
class TripReviewRequest(BaseModel):
    """人工审核旅行计划的请求"""
    action: Literal["approve", "edit", "replan", "reject"] = Field(
        ...,
        description="审核操作",
    )
    draft: Optional[TripPlan] = Field(default=None, description="编辑后的旅行计划")
    feedback: str = Field(default="", description="重新规划时提供的反馈")

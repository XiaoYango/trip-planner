...
class TripAgentPool:
    ...
    #             max_workers=3,
    #             thread_name_prefix="trip-query-agent"
    #         ) as executor:
    #             attraction_future = executor.submit(
    #                 self.attraction_agent.run, attraction_query
    #             )
    #             weather_future = executor.submit(
    #                 self.weather_agent.run, weather_query
    #             )
    #             hotel_future = executor.submit(
    #                 self.hotel_agent.run, hotel_query
    #             )
    #
    ...
    def _build_attraction_query(self, request: TripRequest) -> str:
        ...
        return (
            f"请搜索{request.city}中与“{keywords}”相关的景点，"
            "返回景点名称、地址、经纬度、类别和评分。"
        )
...
def get_trip_planner_agent() -> TripAgentPool:
    """获取多智能体旅行规划系统实例(单例模式)"""
    global _multi_agent_planner

    if _multi_agent_planner is None:
        _multi_agent_planner = TripAgentPool()

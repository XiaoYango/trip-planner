<template>
  <div class="result-container">
    ...
    <a-alert
      v-if="workflowStatus === 'awaiting_approval'"
      type="warning"
      show-icon
      message="旅行计划草案等待确认"
      description="你可以直接批准、进入编辑模式修改后批准，或要求智能体重新规划。"
      style="max-width: 1400px; margin: 0 auto 24px"
    >
      <template #action>
        <a-space>
          <a-button
          ...
          <a-button
            ...
            type="primary"
            :loading="workflowLoading"
            @click="approveEditedPlan"
          >
            提交修改并批准
          </a-button>
          <a-button :disabled="workflowLoading" @click="requestReplan">
            重新规划
          </a-button>
          <a-button danger :disabled="workflowLoading" @click="rejectPlan">
            拒绝
          </a-button>
        </a-space>
      </template>
...
import { resumeTripPlan } from '@/services/api'
import type {
  TripPlan,
  TripPlanResponse,
  TripReviewRequest,
...
const workflowThreadId = ref('')
const workflowStatus = ref<WorkflowStatus | ''>('')
const workflowLoading = ref(false)
let map: any = null

onMounted(async () => {
  const workflowData = sessionStorage.getItem('tripWorkflow')
  if (workflowData) {
    const workflow = JSON.parse(workflowData)
    workflowThreadId.value = workflow.threadId || ''
...
const applyWorkflowResponse = async (response: TripPlanResponse) => {
  if (response.status) {
    workflowStatus.value = response.status
  }
  if (response.thread_id) {
    workflowThreadId.value = response.thread_id
  }

  sessionStorage.setItem('tripWorkflow', JSON.stringify({
    threadId: workflowThreadId.value,
...
const resumeWorkflow = async (review: TripReviewRequest) => {
  if (!workflowThreadId.value) {
    message.error('缺少工作流ID，无法恢复旅行规划')
    return
  }

  workflowLoading.value = true
  try {
    const response = await resumeTripPlan(workflowThreadId.value, review)
    await applyWorkflowResponse(response)

    if (response.status === 'completed') {
      editMode.value = false
      message.success('旅行计划已确认')
    } else if (response.status === 'awaiting_approval') {
      message.info('旅行计划已重新生成，请再次确认')
    } else if (response.status === 'cancelled') {
      message.info('已拒绝当前旅行计划')
    } else {
      message.error(response.message || '旅行规划失败')
    }
  } catch (error: any) {
    message.error(error.message || '恢复旅行规划失败')
  } finally {
...
const approvePlan = async () => {
  await resumeWorkflow({ action: 'approve' })
}

const approveEditedPlan = async () => {
  if (!tripPlan.value) return
  await resumeWorkflow({
    action: 'edit',
    draft: tripPlan.value
  })
}

const requestReplan = async () => {
  const feedback = window.prompt('请输入重新规划要求：', '')
...
const rejectPlan = async () => {
  await resumeWorkflow({ action: 'reject' })
}
